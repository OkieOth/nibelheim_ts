...
export async function delete${currentType.name}ByObjectId(objId: string, dbName: string, collectionName?: string): Promise<number> {
...
%>
export async function delete${currentType.name}ByKey(key: ${typescriptFuncs.printTypescriptType(keyProperty.type, False)}, dbName: string, collectionName?: string): Promise<number> {
    return new Promise(async (resolve, reject) => {
        try {
            const collectionNameToUse = ! collectionName ? "${currentType.name}" : collectionName;
            const db: mongoDb.Db = await mongoConnection.getDb(dbName);
